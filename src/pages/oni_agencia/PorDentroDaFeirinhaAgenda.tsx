...
const PorDentroDaFeirinhaAgenda = () => {
  ...
  useEffect(() => {
    if (clients && clients.length > 0) {
      const client = clients.find(client => 
        client.name.toLowerCase().includes('por dentro') && 
        client.name.toLowerCase().includes('feirinha')
      );
      
      if (client) {
        console.log("Found Por Dentro da Feirinha client:", client);
        setClientId(client.id);
      } else {
        console.error("Por Dentro da Feirinha client not found in clients list");
    ...
    <main className="container-fluid p-0 max-w-full">
      ...
      <div className="container mx-auto p-4 max-w-full">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-2xl font-semibold tracking-tight">Agenda - Por Dentro da Feirinha</h1>
        ...
        <div className={`w-full overflow-x-auto ${isCollapsed ? 'h-[calc(100vh-150px)]' : 'h-[calc(100vh-250px)]'} transition-all duration-300`}>
          {viewMode === "calendar" ? (
            <ContentCalendar
              ...
              clientId="por_dentro_da_feirinha"
              month={selectedMonth}
              year={selectedYear}
...
export default PorDentroDaFeirinhaAgenda;
