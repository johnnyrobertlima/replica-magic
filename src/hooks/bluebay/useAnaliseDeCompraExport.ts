...
export const useAnaliseDeCompraExport = () => {
  ...
  const handleExportAnaliseDeCompra = (groupedItems: GroupedEstoque[], filteredItems: EstoqueItem[]) => {
    try {
      ...
      const fileName = `analise-compra-${new Date().toISOString().split('T')[0]}`;
  ...
  return { handleExportAnaliseDeCompra };
};
