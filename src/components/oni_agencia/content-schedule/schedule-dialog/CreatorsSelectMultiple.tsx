      ...
      <div className="space-y-2">
        <Popover open={open} onOpenChange={setOpen}>
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between text-left" disabled={isLoading}>
          ...
          <PopoverContent className="w-full p-0">
            <Command>
              ...
              <CommandGroup className="max-h-64 overflow-y-auto">
                {collaborators.map(collaborator => <CommandItem key={collaborator.id} value={collaborator.name} onSelect={() => {
                handleSelect(collaborator.id);
              }}>
                    <Check className={cn("mr-2 h-4 w-4", value.includes(collaborator.id) ? "opacity-100" : "opacity-0")} />
                    {collaborator.name}
                  </CommandItem>)}
              </CommandGroup>
            </Command>
        ...
        {value.length > 0 && <div className="flex flex-wrap gap-1 mt-2">
            {value.map(id => <Badge key={id} variant="secondary" className="px-2 py-1 bg-emerald-200">
