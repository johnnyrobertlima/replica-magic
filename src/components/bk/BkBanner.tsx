...
export const BkBanner = () => {
  const [currentBannerIndex, setCurrentBannerIndex] = useState(0);

  const { data: banners, isLoading } = useQuery({
    queryKey: ["active-banners", "bk"],
    queryFn: async () => {
      const { data, error } = await supabase
        .from("banners")
        .select("*")
        .eq("is_active", true)
        .or('page_location.eq.bk,page_location.eq.both')
  ...
  if (!banners || banners.length === 0) {
    return (
      <div className="relative h-[30vh] min-h-[250px] w-full overflow-hidden bg-gradient-to-r from-purple-600 to-blue-500">
