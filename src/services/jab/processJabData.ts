...
/**
 * Process raw pedidos data into organized orders
 */
export function processPedidosData(
  ...
  const orders: JabOrder[] = numeroPedidos.map(numPedido => {
    ...
    let total_saldo = 0;
    let valor_total = 0;
    const items = new Map<string, JabOrderItem & { pedido?: string }>();
...
/**
 * Process data for all pedidos (used in all pedidos fetch)
 */
export function processBatchedPedidosData(
  pedidosDetalhados: any[],
  pessoasMap: Map<number, any>,
  itemMap: Map<string, string>,
  estoqueMap: Map<string, number>,
  ...
  // Group pedidos by pedido number
  const pedidosAgrupados = new Map<string, any[]>();
  pedidosDetalhados.forEach(pedido => {
    const key = pedido.PED_NUMPEDIDO;
    if (!pedidosAgrupados.has(key)) {
